...
for v1, v2, flags, sp, tag, fr, bk in lines:
    ...
    if dist < 70:
        ...
        print(f"d={dist:3.0f} ({x1},{y1})-({x2},{y2}) flags={flags} special={sp} front floor/ceil={fs[0]}/{fs[1] if fs else None} back={bs[0] if bs else None}/{bs[1] if bs else None} midtex={sides[fr][4].strip(b'\0')}")
PY
